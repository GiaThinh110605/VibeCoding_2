-- Library Management System Database Schema for PostgreSQL
...
CREATE TABLE users (
    ...
    email VARCHAR(100),
    phone VARCHAR(20),
    role VARCHAR(20) CHECK (role IN ('admin', 'librarian')) NOT NULL,
...
CREATE TABLE readers (
    ...
    status VARCHAR(20) CHECK (status IN ('active', 'locked', 'expired')) DEFAULT 'active',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TRIGGER update_book_titles_updated_at BEFORE UPDATE ON book_titles
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_borrowing_transactions_updated_at BEFORE UPDATE ON borrowing_transactions
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
